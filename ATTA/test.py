...
from preprocess_shhs import get_double_eeg_datasets,get_double_eog_datasets
...
def get_args():
    ...
    parser.add_argument("--data_path", '-d', type=str, default="../SHHS/polysomnography/edfs/shhs1/npz/", help="Path to the data.")
    ...
    parser.add_argument("--log_file", type=str, default='./log/153toSHHS/153toSHHS.txt', help="Path to the log file.")
...
double_eeg_dataset_list=get_double_eeg_datasets(npz_file_pairs,stride=35)
...
def evaluate():
    ...
    eeg_model_path = pth_dir + 'model_sleepedf_eeg.pth' # Path where the model is saved.
    eog_model_path = pth_dir + 'model_sleepedf_eog.pth'
    ...
    eeg_dataset_list_total = data_split(double_eeg_dataset_list, total_num=file_num)
    eog_dataset_list_total = data_split(double_eog_dataset_list, total_num=file_num)
        ...
        for (x_eeg, y_eeg), (x_eog, y_eog) in zip(eeg_test_loader, eog_test_loader):
            ...
            for step in range(1, steps + 1):
                ...
                with torch.no_grad():
                    ...
                    keys_eeg = (eeg_model_r(x_eeg, feature_extract=True) + eeg_model_a(x_eeg, feature_extract=True)) / 2
                    keys_eog = (eog_model_r(x_eeg, feature_extract=True) + eog_model_a(x_eeg, feature_extract=True)) / 2
                    keys_eeg=keys_eeg.detach().cpu().numpy()
                    keys_eog=keys_eog.detach().cpu().numpy()
    ...
    ShowConfusionMatrix(label_all_subject['t_label'], label_all_subject['p_label'], savePath='./conf_matrix/153toSHHS', title=title_name)
