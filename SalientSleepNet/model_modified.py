...
class SalientSleepNet_without_up(nn.Module):
    def __init__(
            self,
            window_size: int = 35,
            channel:str = 'eeg'
    ):
        super(SalientSleepNet_without_up, self).__init__()
        self.l = 3000
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Integrate the outputs of each channel.
        if self.channel == 'eeg':
            merge = eeg
...
if __name__ == '__main__':
    ...
    batch_size =10
    seq_len = 35
    model = SalientSleepNet_without_up(seq_len)
